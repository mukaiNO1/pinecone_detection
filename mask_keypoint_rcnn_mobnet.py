...
class MaskKeypointRCNN(FasterRCNN):
    def __init__(self, backbone, num_classes=None,
        ...
        if mask_head is None:
            mask_layers = (out_channels, out_channels, out_channels, out_channels)
            mask_dilation = 1
            mask_head = MaskRCNNHeads(out_channels, mask_layers, mask_dilation)
        if mask_predictor is None:
            mask_predictor_in_channels = out_channels  # == mask_layers[-1]
            mask_dim_reduced = out_channels
...
def maskkeypointrcnn_resnet34_fpn(num_classes=2, num_keypoints=3,
                                  pretrained_backbone=True, **kwargs):
    backbone = resnet_fpn_backbone('wide_resnet50_2', pretrained_backbone)
    model = MaskKeypointRCNN(backbone, num_classes=num_classes, num_keypoints=num_keypoints, **kwargs)
    return model

def maskkeypointrcnn_mobilenet(num_classes=2, num_keypoints=2, pretrained=False,pretrained_backbone=True,
    ...
    model = MaskKeypointRCNN(backbone, num_classes=num_classes, rpn_anchor_generator=AnchorGenerator(anchor_sizes, aspect_ratios), num_keypoints=num_keypoints, **kwargs)
    return model
...
if __name__ == '__main__':
    ...
    #mkrcnn = maskkeypointrcnn_resnet50_fpn(num_keypoints=3, pretrained=False).to(device)
    mkrcnn = maskkeypointrcnn_mobilenet(num_keypoints=3).to(device)
    #test predicting
    mkrcnn.eval()
    img = [torch.rand(3, 640, 480).to(device)]
    fps = []
    for i in range(100):
        start = time.time()
        res = mkrcnn(img)
    ...
    print("fps mean:{}".format(np.mean(fps)))
    mkrcnn.eval()
    tensor = (torch.rand(1, 3, 640, 480).to(device),)

    # 分析FLOPs
    flops = profile(mkrcnn, tensor)
    print("FLOPs: ", flops)
    # test training
    # mkrcnn.train()
    ...
    #
    #
    # total_params = sum(p.numel() for p in mkrcnn.parameters())
    # print(f'{total_params:,} total parameters.')
    # print(f'{total_params / (1024 * 1024):.2f}M total parameters.')
    # total_trainable_params = sum(
    #     p.numel() for p in mkrcnn.parameters() if p.requires_grad)
