...
def predict(image_cv2, model, transforms, device,
    ...
    for score, bbox, keypoints, keypoint_scores, mask in zip(pred["scores"], pred["boxes"], pred["keypoints"], pred["keypoints_scores"], pred["masks"]):
        if score >= box_score_thre:
            ...
            canvas[:, :, 0] = np.where(mask >= mask_thre, random.randint(0, 255), canvas[:, :, 0])
            canvas[:, :, 1] = np.where(mask >= mask_thre, random.randint(0, 255), canvas[:, :, 1])
            #canvas[:, :, 2] = np.where(mask >= mask_thre, random.randint(0, 255), canvas[:, :, 2])
            ...
            for i, (kp, kp_score) in enumerate(zip(keypoints, keypoint_scores)):
                ...
                if visibility >= 0.5 and kp_score >= kp_score_thre:
                    cv2.circle(canvas, (x, y), 2, (0, 255, 0), 2)
...
@torch.no_grad()
def predict_eval(image_cv2, model, transforms, device,
            kp_names=("l", "r"),
            box_score_thre=0.5, kp_score_thre=0, mask_thre=0.5,
            show=False):
    image = cv2.cvtColor(image_cv2, cv2.COLOR_BGR2RGB)
    ...
    canvas = np.copy(image_cv2)
    results = []
    # for score, bbox, keypoints, keypoint_scores, mask in zip(pred["scores"], pred["boxes"], pred["keypoints"],
    #                                                          pred["keypoints_scores"], pred["masks"]):
    #     if score >= box_score_thre:
    #         res = {"bbox": None, "mask": None, "keypoints": {kp_name: None for kp_name in kp_names}}
    #         xmin, ymin, xmax, ymax = map(int, bbox)
    #         cv2.rectangle(canvas, (xmin, ymin), (xmax, ymax), (0, 255, 255), 2)
