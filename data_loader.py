...
def get_dataloader(img_dir, lab_path, kp_names,
    ...
    dataset = MaskKeypointDataset(img_dir=img_dir, lab_path=lab_path, keypoint_names=kp_names, transforms=get_transforms(train=False))
    if val_split == 0.0:
        ...
        dataloader_train = DataLoader(dataset_train, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, collate_fn=MaskKeypointDataset.collate_fn)
        dataloader_val = DataLoader(dataset_val, batch_size= 1, shuffle=shuffle, num_workers=num_workers, collate_fn=MaskKeypointDataset.collate_fn)
